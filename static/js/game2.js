...
window.onload = function(){
  ...
  function startGame(){
      ...
      c.font = "1em Arial";
      function endGame() {
        // Display a game over message or perform any other end game actions
        ...
        // window.location.href = "/game_over"; // Assuming "/game_over" is the URL for the game over page
    }
      function stoperror() {
          return true;
      }  
          ...
          for(var j = _enemies.length-1; j >= 0; j--){
              for(var l = _bullets.length-1; l >= 0; l--){
                  if(collision(_enemies[j], _bullets[l])){
                      ...
                      _bullets.splice(l, 1);
                      score++;
                      // updateScore(score); // Call updateScore when the player destroys an enemy
                  }
              }
          }
          ...
          for(var hh = _healthkits.length-1; hh >= 0; hh--){
              for(var hhh = _bullets.length-1; hhh >= 0; hhh--){
                  if(collision(_healthkits[hh], _bullets[hhh])){
                      ...
                      health += 10;
                       // Call updateScore when the player collects a health kit
                  }
              }
          } 
      }
...
function sendScoreToBackend(score, username) {
  console.log("not")
    try {
        ...
        console.log("last")
        console.log(data)
        // Set up the event listener to handle response
        xhr.onload = function() {
            if (xhr.status === 200) {
              console.log("fittest")
                console.log("Score added successfully");
            } else {
                console.error("Failed to add score. Status code: " + xhr.status);
            }
        };

        // Set up error handling
        xhr.onerror = function() {
            console.error("An error occurred while sending the score to the backend.");
        };

        // Send the request
        xhr.send(data);
    } catch (error) {
        console.error("An error occurred in sendScoreToBackend():", error);
    }
}

// Call updateScore whenever the score changes in your game logic
function updateScore(newScore) {
  console.log("work")
    try {
        ...
        score = newScore;
        // Update the score display on the canvas or wherever you want to display it
        // For example:
        c.fillStyle = "white";
        c.fillText("Score: " + score, innerWidth - 100, 20);
        console.log("working")
        // Pass the username and score to sendScoreToBackend function
        sendScoreToBackend(score);
    } catch (error) {
        console.error("An error occurred in updateScore():", error);
    }
}
