...
c.pack(side='top',fill='both',expand='true',anchor='center')
...
def start_game(event=None):
    # Remove the binding for the Enter key
    win.unbind('<Return>')
    
    # Create game elements and start the game loop
    create_eggs()
    move_eggs()
    catch_check()
def submit_final_score(username, final_score):
    # Define the URL of the Flask application route
    url = 'http://localhost:5000/submit_egg_score'  # Adjust the URL as needed
...
def egg_dropped(egg):
    ...
    lose_life()
    if live_remain==0:
        submit_final_score('rrrrrrrrrr', score)
